...
class MySpider(CrawlSpider):
  ...
  start_urls = ["https://play.google.com/store/apps/details?id=com.facebook.orca"]
...
##        item["Item_name"] = ''.join(titles.select('//*[@class="document-title"]/div/text()').extract())
        item["Updated"] = ''.join(titles.select('//*[@class="review-date"]/text()').extract())
...
##        item["Review_number"] = ''.join(titles.select('//*[@class="reviews-num"]/text()').extract())
        item["Description"] = ''.join(titles.select('//*[@class="review-body"]/text()').extract())
